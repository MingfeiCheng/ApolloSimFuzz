...
from scenario_elements.agents.waypoint_walker import WaypointWalkerAgent, WaypointWalkerConfig

class SMWaypointWalker(SubScenarioManager[WaypointWalkerConfig]):

    name: str = 'openscenario_waypoint_walker'

    def __init__(
        self,
        id: str,
        configs: List[WaypointWalkerConfig],
        sandbox_container_name: str,
        scenario_dir: str = None,
        terminate_on_failure: bool = False,
        debug: bool = False
    ):
        super(SMWaypointWalker, self).__init__(
            id = id,
            configs = configs,
